...
try {
  ...
  await page.evaluate(() => {
    localStorage.removeItem("orbita.v3");
    localStorage.removeItem("orbita.v3.backup");
  ...
  assert.equal(
    await page.evaluate(
      () => JSON.parse(localStorage.getItem("orbita.v3")).legacyBest,
    ),
    900,
  );
  ...
  await page.evaluate(() => localStorage.setItem("orbita.v3", "{corrupt"));
  await page.reload();
  await expect(
    page.getByRole("button", { name: "Explorar campaña" }),
  ).toBeVisible();
