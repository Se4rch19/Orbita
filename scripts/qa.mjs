...
try {
  ...
  await page.goto("http://localhost:4173/?qa=0.2");
  await expect(
  ...
  let save = await page.evaluate(() =>
    JSON.parse(localStorage.getItem("orbita.v2")),
  );
  assert.equal(save.tutorial, true);
  assert.equal(save.runs, 0);
  assert.equal(save.totalLights, 0);
  checks.push(
    "independent 30-second interactive tutorial, no progression rewards",
  );
